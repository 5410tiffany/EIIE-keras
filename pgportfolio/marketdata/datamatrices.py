...
class DataMatrices:
    def __init__(self, start, end, period, batch_size=50, volume_average_days=30, buffer_bias_ratio=0,
        ...
        print(start, end, period, batch_size, volume_average_days, buffer_bias_ratio,
                 market, coin_filter, window_size, feature_number, test_portion,
                 portion_reversed, online, is_permed)
        if market == "poloniex":
            self.global_data = self.__history_manager.get_global_panel(start,
                                                                         self.__end,
                                                                         period=period,
                                                                         features=type_list)
        else:
        ...
        self._PVM = pd.DataFrame(index=self.global_data.minor_axis,
                                  columns=self.global_data.major_axis)
        self._PVM = self._PVM.fillna(1.0 / self.__coin_no)

        self._window_size = window_size
        self._num_periods = len(self.global_data.minor_axis)
        ...
        self.replay_buffer = rb.ReplayBuffer(start_index=self._train_ind[0],
                                               end_index=end_index,
    ...
    def append_experience(self, online_w=None):
        ...
        self.replay_buffer.append_experience(appended_index)
    ...
    def next_batch(self):
        ...
        batch_size
        """
        batch = self.__pack_samples([exp.state_index for exp in self.replay_buffer.next_experience_batch()])
        return batch

    def __pack_samples(self, indexs):
        indexs = np.array(indexs)
        last_w = self._PVM.values[indexs-1, :]

        def setw(w):
            self._PVM.iloc[indexs, :] = w
    ...
    def get_submatrix(self, ind):
        return self.global_data.values[:, :, ind:ind+self._window_size+1]
