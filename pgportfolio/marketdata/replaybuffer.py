...
class ReplayBuffer:
    def __init__(self, start_index, end_index, batch_size, is_permed, coin_number, sample_bias=1.0):
        ...
        self.experiences = [Experience(i) for i in range(start_index, end_index)]
    ...
    def append_experience(self, state_index):
        self.experiences.append(Experience(state_index))
        logging.debug("a new experience, indexed by %d, was appended" % state_index)

    def sample_idx(self, start, end, bias):
        """
    ...
    def next_experience_batch(self):
        ...
        if self.__is_permed:
            for i in range(self.__batch_size):
                batch.append(self.experiences[self.sample_idx(self.experiences[0].state_index,
                                                              self.experiences[-1].state_index,
                                                              self.__sample_bias)])
        else:
            batch_start = self.sample_idx(0, len(self.experiences) - self.__batch_size, self.__sample_bias)
            batch = self.experiences[batch_start:batch_start+self.__batch_size]
        return batch
